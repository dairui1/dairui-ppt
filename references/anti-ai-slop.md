...
## 中文 prose 黑名单（文案侧的 AI slop）

LLM 写中文 deck 文案时会反复落到几个固定的 tic 上。这些句子结构本身就在告诉读者"这是 AI 写的"：

| 不要写 | 为什么 | 改成 |
|---|---|---|
| `X · Y · Z 都还在` / `信息密度 · 动画 · 叙事都齐了` 这种中点列举 | 三段中点分隔 = LLM 招牌排版，像 marketing deck 副标 | 直接陈述："信息没丢" |
| `被 X 接住、被 Y 包起来、被 Z 按下` 这种排比被动 | 4 个被动动词排成一串太工整，真人写作不会让每个组件都用同样语法槽 | 每个组件一句正常的话 |
| 给每个技术组件**统一**拟人化（"SQLite 守门员"、"Runtime 的天下"、"Model 按下") | 一处拟人有趣，**整段**这样就 AI 了——LLM 在"炫巧"地选可爱动词 | 选一个最贴切的强动词，其它平铺 |
| 副标用一句话把整条 pipeline 重述一遍 | "punchline summary" 是 LLM tic 的典型，标题已经说过的不要再压缩成一句重述 | 副标说**不同**信息，或直接砍掉 |
| 一串"cute 动词"链式串联：翻 / 包起来 / 接住 / 按下 / 走完 | LLM 在表演"我能想到很多生动动词" | 一个强动词就够，其余用普通词 |
| `一份 X、一份 Y` 这种伪并列起句 | 像广告 copywriter 起范儿 | 直接说要做什么 |

**反例 → 正例**：

```
✗ 每幕只演一站 · 左讲右演 · 胶囊从 TUI 出发，被 SQLite 接住、被 Runtime 包起来、被 Model 按下，最后沿反馈环把 status 翻成 complete。
   ↑ 五个 AI tic 同时出现：中点三段、排比被动、统一拟人、pipeline 重述、cute 动词串

✓ 一幕一站。左边写说明，右边放动画。
   ↑ 短陈述，不重复标题已说的。
```

**自检触发词**——副标 / 段落 / colophon 写完后扫一遍：
