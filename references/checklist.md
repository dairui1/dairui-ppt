...
**做法**:
- 瑞士主题的 golden source 就是 `assets/template-swiss.html` 自身,但要以**实际页面用法**为准,不要只看未使用的 CSS helper
...
**做法**：
- **生成 PPT 前,必须先 `Read` 当前风格对应模板**:风格 A 读 `assets/template-magazine.html`,风格 B 读 `assets/template-swiss.html`,确认 layouts 里用到的类都已定义
...
```
预检(生成前)
  □ 已读过 template-magazine.html 的 <style>,确认所需类都存在
